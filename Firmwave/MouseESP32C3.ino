...
const char* WIFI_SSID_STA = "SEU_WIFI";
const char* WIFI_PASS_STA = "SUA_SENHA";

void sendLog(const String &msg) { Serial.println("[INFO] " + msg); }
void sendError(const String &msg) { Serial.println("[ERROR] " + msg); }
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    sendLog("Wi-Fi conectado: " + ip.toString());
  } else {
    sendError("Falha ao conectar Wi-Fi, iniciando AP.");
    WiFi.softAP(WIFI_SSID, WIFI_PASS);
    IPAddress IP = WiFi.softAPIP();
    sendLog("Modo AP ativo: " + IP.toString());
  }
...
void loop() {
  ...
  if (digitalRead(BTN_LEFT) == LOW) { Mouse.click(MOUSE_LEFT); delay(150); }
  if (digitalRead(BTN_RIGHT) == LOW) { Mouse.click(MOUSE_RIGHT); delay(150); }
  if (digitalRead(BTN_SCROLL) == LOW) { Mouse.move(0, 0, 3); delay(150); }
...
void handleCommand(String cmd) {
  ...
  else sendError("Comando desconhecido: " + cmd);
}
